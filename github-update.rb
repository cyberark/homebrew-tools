...
class GithubUpdate
  ...
  def self.getLatestRelease(repo)
    ...
    latest_release_uri = URI.parse("https://api.github.com/repos/#{repo}/releases/latest")
    latest_release_response = self.get_with_redirect(latest_release_uri)
    latest = JSON.parse(latest_release_response.body)
    artifacts_json = latest["assets"]
    ...
    if hashes_artifact_matches.empty?
      raise "SHA256SUMS asset not found attached to github release #{latest_release_uri}"
    end
